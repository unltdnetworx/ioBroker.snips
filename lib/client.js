...
function MQTTClient(adapter) {
    ...
    function send2Server(id, state, cn) {
        ...
        case 'inject_broadcast':
            ...
            break;
        case 'inject_genre':
			client.publish(id, JSON.stringify({"operations":[["add",{"de.iobroker.genres":[state]}]]}), {qos: adapter.config.defaultQoS, retain: adapter.config.retain});
            break;
        case 'inject_interpret':
			client.publish(id, JSON.stringify({"operations":[["add",{"de.iobroker.interprets":[state]}]]}), {qos: adapter.config.defaultQoS, retain: adapter.config.retain});
			break;
		case 'sound' :
            if (state) {
