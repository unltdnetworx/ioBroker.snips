                        ...
                        switch (result.slots[i].slotName) {
                            ...
                            case 'room':
                                adapter.getState('devices.' + roomid + '.enforceSameRoom', function (err, state) {
                                    if(state.val) {
                                        var dp = adapter.getObject('snips.0.devices.default.enforceSameRoom','rooms').enumNames.join(', ');
                                        adapter.log.error("DP: " + dp);
                                    } else if(!slotRoom || slotRoom == '') {
                                        slotRoom = result.slots[i].value.value;
                                        adapter.log.error(slotRoom);
                                    } else {
